...
- [x] 두음법칙 최종 결정 (SPEC.md 갱신) — v0.5, "적용함"으로 확정
- [x] 답 입력 UI → SPEC.md 3절 비교 규칙으로 정답 목록과 대조 — `GuessForm.jsx` 신규, `submit_guess` RPC 호출
- [x] 통과 시 `matched_answer_id` 기록 — `submit_guess` RPC 내부에서 처리 (Supabase 쪽에 이미 구현되어 있었음, 프론트는 호출만)
- [x] `OPEN → SOLVED` 전이를 원자적으로 처리 (동시 제출 레이스 방지) — RPC 내부 로직, `already_solved` 필드로 늦게 도착한 제출을 구분
- [x] 오답 제출은 무제한 허용, 단 모든 시도 기록 — `turn_attempts`에 RPC가 자동 기록(프론트에서 별도 insert 없음)

**완료 조건 충족**: 정답을 맞히면 통과 처리되고, 다음 사람이 이어야 할 시작 글자가 화면에 확정되어 나온다.
**참고**: `next_start_chars`를 다음 턴 업로드 시 실제로 강제하는 것은 Phase 5 스코프 — 지금은 화면에 안내만 되고 업로드 단에서 강제 검증은 아직 없음.
